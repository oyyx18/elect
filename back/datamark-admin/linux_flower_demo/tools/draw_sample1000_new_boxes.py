#!/usr/bin/env python3
"""Draw Pascal VOC boxes on images in sample-1000-new dataset."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Draw Pascal VOC boxes on sample-1000-new images and save copies."
    )
    parser.add_argument(
        "--input",
        default=str(
            Path(__file__).resolve().parents[1] / "allDatasets" / "sample-1000-new"
        ),
        help="Input dataset root (default: linux_flower_demo/allDatasets/sample-1000-new)",
    )
    parser.add_argument(
        "--output",
        default=str(
            Path(__file__).resolve().parents[1] / "allDatasets" / "sample-1000-new-boxed"
        ),
