...
import { deepMapData } from '@/utils/common';
...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
  ...
  const { apiFn, apiParams, immediate, showTotal, sign } = config;
...
export function useTableOperate<T extends TableData = TableData>(data: Ref<T[]>, getData: () => Promise<void>) {
  ...
  function handleAdd(id: T['id']) {
    if (id) {
      operateType.value = 'add';
      editingData.value = { ...editingData.value, parentId: id };
      openDrawer();
    } else {
      // if(editingData.value?.parentId) {
      //   delete editingData.value
      // }
      editingData.value = { ...editingData.value, parentId: '' };
      operateType.value = 'add';
      openDrawer();
    }
  }
  ...
  function handleEdit(id: T['id'], parentId?: any) {
    operateType.value = 'edit';
    if (parentId) {
      const children = data.value.find(item => item.id === parentId).children || null;
      const findItem = children.find(item => item.id === id) || null;
