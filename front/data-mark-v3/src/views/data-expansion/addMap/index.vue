...
import { getResultDataSetSave } from "@/service/api/ano";

import {NCascader, StepsProps} from "naive-ui";
import { getDicts } from "@/service/api";
import { useFormRules, useNaiveForm } from "@/hooks/common/form";
import {fetchDataSetAdd, getSelectDataSetDictList} from "@/service/api/dataManage";
const infoObj = ref({
...
const handleCreImp = async () => {
  await validate();
  visible.value = true;
  const res = await getResultDataSetSave({
...
onMounted(async () => {
  ...
  dictMapList.value = recursionData(res.data);
});
</script>
...
<template>
  <div class="wrap_container flex_col_start relative">
    ...
    >
      ...
      <div
        class="wrap_timeline w-full flex justify-center items-center box-border py-12px"
        v-if="false"
      >
        <n-steps
      ...
      <div class="item_card">
        <n-card title="基本信息">
          ...
          >
            ...
            <n-form-item label="数据集类型" path="dataTypeId" class="w-[70%]">
              <n-cascader
            ...
            <n-form-item label="数据类型" path="type" class="mt-[4px]" v-if="false">
              <n-radio-group v-model:value="model.dataType" name="type">
                <n-radio-button
                  v-for="(val, idx) of mapObj.typeList"
                  :value="val.id"
                  @click="handleRadioClick('dataType', val)"
                  >{{ val.dictLabel }}</n-radio-button
                >
              </n-radio-group>
            </n-form-item>
            ...
            <n-form-item label="保存位置" path="version" v-show="false">
              <n-radio-group
                v-model:value="formInfo.model.anomarkTemp"
                name="radiogroup"
              >
                <n-space>
    ...
    <div class="footer flex_start">
      <n-button type="info" @click="handleCreImp()">创建</n-button>
    ...
    <div v-show="visible" class="mask-layer">
      <div class="loading-spinner">
        <!-- 这里可以放置任何你想要的加载动画 -->
        <NSpin size="large" description="保存中... 请稍等" />
      </div>
    </div>
  </div>
</template>
