...
type OperateType = 'result' | 'detail' | 'report' | 'view';
...
async function handleOperate(type: OperateType, row: any) {
  ...
  if (type === 'report') {
    reportShowModal.value = true;
  }
  if (type === 'view') {
    router.push({
      name: "model_info", query: { taskId: row.taskId }
    })
  }
}
          ...
          <NFlex justify="space-between" class="wrap-container">
            ...
            >
              ...
              <div class="item_arrow" v-if="index !== aInfoList.length - 1">
                <div class="flow-arrow"><span class="aibp-custom-icon aibp-custom-icon-arrow"><svg width="24"
                                                                                                   height="24"><path
                  fill="#B8BABF" d="m8.053 3 9.192 9.192L8 21.437v-5.253l3.79-3.79L8 8.603V3.052L8.053 3Z"></path></svg></span>
                </div>
              </div>
            </div>
          </NFlex>
