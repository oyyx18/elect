...
const fileValidation2 = (file: any) => {
  ...
  const isBool = allowedExtensions.includes(
  ...
  const MAX_FILE_SIZE = 4 * 1024 * 1024 * 1024; // 4G
  if (file.size > MAX_FILE_SIZE) {
    file.status = "error";
    file.errMessage = "文件大小不能超过 4G";
    return false;
  }
};

const fileOnChange = (_uploadFile: UploadFile, uploadFiles: UploadFiles) => {
  console.log("_uploadFile: ", _uploadFile);
  console.log("uploadFiles: ", uploadFiles);
  if (importMode.value === "0-0") {
    uploaderFiles.value = _.uniqBy(uploadFiles, "name").map(
      (item: any, index) => {
  ...
  if (importMode.value === "0-1") {
    ...
    const existFile = uploaderFiles.value.find(
      (item: any) => item.name === _uploadFile.name
    );
    if (existFile) {
    ...
    uploaderFiles.value = _.uniqBy(uploadFiles, "name").map(
      (item: any, index) => {
        ...
        item.isHover = false;
        item.percentage = 0;
        item.chunks = Array.from(
          { length: Math.ceil(item.size / CHUNK_SIZE) },
          (_, i) => ({
            index: i,
            uploaded: false,
          })
...
const removeFile = (row: any) => {
  ...
  const fileList =
    markStatus.value === "0"
      ? uploaderFiles.value.filter(
          (val: { status: string }) => val.status === "finished"
        )
      : uploaderFiles.value.filter(
          (val: { status: string }) => val.status === "finished" && isImg(val)
        );
  emit("submitted", {
    fileList,
  });
...
const maxConcurrentRequests = 30; // 最大并发请求数
...
const handleProgress = (
  ...
  const fileItem = uploaderFiles.value[fileIdx].raw;
  const percentage = getPercentage(fileItem);
  if (event.lengthComputable) {
    if (percentage == 100) {
      ...
      loading.value = false;
      isUpSuccess.value = false;
      const fileList = uploaderFiles.value.filter(
        (val: { status: string }) => val.status === "finished"
      );
      emit("submitted", {
        fileList,
...
const uploadFileInChunks = async (file: File, fileIdx: any) => {
  ...
  for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
    ...
    const fileIndex = uploaderFiles.value.findIndex(
      (val: any) => val.name === file.name
    );
  ...
  const fileItem = uploaderFiles.value.find(
    (val: any) => val.name === file.name
  );
  fileItem.status = "finished";
  fileItem.errMessage = `文件上传完成！`;
...
const handleFileUpload = (files) => {
  ...
  files.forEach(async (file, fileIdx) => {
    ...
    // const isExist = await checkFileExist(fileHash);
    // if (isExist) {
    //   ElMessage.success(`${file.name} 文件已存在，秒传成功！`);
    //   continue;
    // }
    // const uploadedChunks = await checkUploadedChunks(fileHash, file.name);
    // const chunks = sliceFile(file);
    // for (let i = 0; i < chunks.length; i++) {
    //   if (!uploadedChunks.includes(i)) {
    //     const result = await uploadChunk(chunks[i], i, fileHash, file.name);
    //     if (result && result.success) {
    //       console.log(`分片 ${i} 上传成功`);
    //     } else {
    //       console.log(`分片 ${i} 上传失败`);
    //     }
    //   } else {
    ...
    await uploadFileInChunks(file, fileIdx);
  });
};

const startUpload = async () => {
  const files = uploaderFiles.value
    .filter((val) => val.status !== "finished")
    .map((item) => item.raw);
  ...
  files.forEach(async (file: File) => {
    initFileProgress(file);
    // const fileIdx = uploaderFiles.value.findIndex(
    //   (val: any) => val.name === file.name
    // );
    // const totalChunks = Math.ceil(file.size / CHUNK_SIZE);
    // const chunkList = sliceFile(file, { totalChunks, fileIdx });
  ...
  const generator = asyncChunkGenerator(files, 2);
  await runChunk(generator);
...
const asyncUploadChunk = async (file: File) => {
  return new Promise(async (resolve, reject) => {
    ...
    const generator = concurrentApiChunkCallGenerator(chunkList, 2);
    await runChunk(generator);
    const fileItem = uploaderFiles.value.find(
      (val: any) => val.name === file.name
    );
      ...
      axios
        ...
        .then((res) => {
          if (res.data.data) {
            ...
            uploaderFiles.value[row.fileIdx].chunks[row.chunkIndex].uploaded =
              true;
...
function* concurrentApiChunkCallGenerator(
  urls: any,
  maxConcurrency = 5,
  sign: string
) {
...
<template>
  <NDrawer v-model:show="visible" display-directive="show" width="540">
        ...
        <div class="wrap-upload mt-24px w-full">
          ...
          >
            ...
            <div v-show="importMode === '0-1'" class="">
              ...
              >
              <NP depth="3" style="margin: 8px 0 0 0">
                1. 文件类型为zip/tar.gz/rar
                <br />
              </NP>
              <NP depth="3" style="margin: 8px 0 0 0"
                >2. 为了保证最佳性能，每次最多可上传2000个文件</NP
              >
            </div>
          </ElUpload>
        </div>
        <div class="w-full overflow-y-auto !flex-1">
          ...
          >
            <template #default="{ item }">
              <div
                class="h-auto w-full flex flex-col items-start justify-start"
              >
                <div
                  :key="item.key"
                  ...
                  <NPopover trigger="hover" placement="left">
                    <template #trigger>
                      ...
                      <div class="w-90% flex items-center justify-start">
                        ...
                        <NButton
                          v-show="item.status && item.status === 'finished'"
                          quaternary
                          type="primary"
                        >
                          已完成
                        ...
                        <NButton
                          v-show="item.status && item.status === 'error'"
                          quaternary
                          type="error"
                          >失败</NButton
                        >
                    ...
                    <div
                      ...
                      :style="{
                        color:
                          item.status && item.status === 'error' ? 'red' : '',
                      }"
                    >
                      {{ item.errMessage }}
                    </div>
                  ...
                  <NPopover trigger="hover" placement="left">
                    <template #trigger>
                      <div class="w-90% flex-col items-center justify-center">
                        ...
                        <div
                          v-show="
                            getPercentage(item.raw) != 100 &&
                            getPercentage(item.raw) != 0
                          "
                          class="w-90%"
                        >
                          <NProgress
                            type="line"
                            :percentage="getPercentage(item.raw)"
