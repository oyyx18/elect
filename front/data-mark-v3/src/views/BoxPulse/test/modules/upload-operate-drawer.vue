...
const imgToolText = computed(() => {
  ...
  return uploaderFilesLength === 0 ? '本次上传文件数: 0, 已完成传输数: 0' : `本次上传文件数: ${uploaderFilesLength}, 已完成传输数: ${uploadedCountValue}/${totalFilesCountValue}`;
});
...
const fileValidation = (file: any) => {
  ...
  if (file.size > MAX_FILE_SIZE) {
    file.status = 'error';
    file.errMessage = '文件大小不能超过 30MB';
    console.error('文件大小不能超过 30MB');
    return false;
  }
};
...
const fileOnChange = (_uploadFile: UploadFile, uploadFiles: UploadFiles) => {
  ...
  if (importMode.value === '0-0') {
    uploaderFiles.value = _.uniqBy(uploadFiles, 'name').map((item: any, index) => {
      ...
      const postfix = item.name.split('.')[length - 1].toLowerCase();
  ...
  if (importMode.value === '0-1') {
    ...
    const uploadedCountValue = uploaderFiles.value.filter((val: any) => val.status === 'finished').length;
    const totalFilesCountValue = totalFilesCount.value;
    tooltipText.value = uploaderFilesLength === 0 ? '本次上传文件数: 0, 已完成传输数: 0' : `本次上传文件数: ${uploaderFilesLength}, 已完成传输数: ${uploadedCountValue}/${totalFilesCountValue}`;
  }
};
...
const handleExceed: UploadProps['onExceed'] = () => {
  window.$message?.error?.('黑白盒测试：最多只能上传一个文件(注： 文件类型只允许为tar类型！)');
};
...
const asyncApiChunkCall = async (row: any, sign: string) => {
  const uploadUrl = `${import.meta.env.VITE_SERVICE_BASE_URL}/bigFileUpload/uploadChunk1`;
  const formData = new FormData();
    ...
    <NDrawerContent title="上传" :native-scrollbar="false" closable>
      <div class="h-full w-full flex flex-col items-start justify-start">
        ...
        <div class="wrap-upload mt-24px w-full">
          <ElUpload ref="uploadRef" v-model:file-list="uploaderFiles" drag action="#" :limit="1" :auto-upload="false"
            :on-change="fileOnChange" :on-exceed="handleExceed" :on-success="handleSuccess"
