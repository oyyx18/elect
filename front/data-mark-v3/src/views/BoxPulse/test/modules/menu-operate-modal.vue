    ...
    <NScrollbar class="h-300px pr-20px">
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left">
        <NGrid responsive="screen" item-responsive class="ml-24px">
          <NFormItemGi span="24 m:24" label="创建时间:" path="createTime" class="h-42px">
            {{ model.createTime }}
          </NFormItemGi>
          <NFormItemGi span="24 m:24" label="完成时间:" path="updateTime" class="h-42px">
            {{ !!model.updateTime ? model.updateTime : "-" }}
          </NFormItemGi>
          <NFormItemGi span="24 m:24" label="任务名称:" path="taskInputName" class="h-42px">
            {{ model.taskName ?? "-" }}
          </NFormItemGi>
          <NFormItemGi span="24 m:24" label="任务类型:" path="taskName" class="h-42px">
            {{ model.taskName ?? "-" }}
          </NFormItemGi>
          <NFormItemGi span="24 m:24" label="任务状态:" path="taskStat" class="h-42px">
            {{ model.taskStat ?? "-" }}
          </NFormItemGi>
          <!-- acc: 攻击前的准确率 -->
          <NFormItemGi span="24 m:24" label="攻击前的准确率:" path="acc" class="h-42px">
            {{ model?.acc ?? "-" }}
          </NFormItemGi>
          <!-- adv_acc:攻击后的准确率 -->
          <NFormItemGi span="24 m:24" label="攻击后的准确率:" path="adv_acc" class="h-42px">
            {{ model?.adv_acc ?? "-" }}
          </NFormItemGi>
        </NGrid>
      </NForm>
